...
ratescales=(1e-3) #1e-4 2e-4 4e-4 8e-4 1e-3 2e-3
...
do
    ...
    do
        ...
        # python test.py --model /users/ll/models/llama2-7B/ --workload-type maf1 --rate-scale $i --scheduler-policy $j --tensor-parallel-size 2  >output/test_${i}_${j}_tp2.out
        python test.py --model /users/ll/models/llama2-7B/ --workload-type maf1 --rate-scale $i --scheduler-policy $j --strict-stop >output/test_${i}_${j}.out
