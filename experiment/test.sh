...
policies=(fcfs interleave sjmlfq emlfq) #fcfs interleave sjmlfq emlfq
for i in ${ratescales[*]};
do
    for j in ${policies[*]};
    do
        echo rate scale $i, policy $j
        python test.py --model /users/zyh/models/llama2-7B/ --workload-type maf1 --rate-scale $i --policy $j >output/test_${i}_${j}.out
